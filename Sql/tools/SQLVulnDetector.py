...
def VulnMain(url_list,thread):
    colorprint.colorprint(f"Total website {len(url_list)}")
    global vulnerable
    ...
    while(i < len(url_list)):
        ...
        i += thread
        threads = []
    with open("results/sql.txt", "w") as file:
        file.writelines("%s\n" % url for url in vulnerable)
    colorprint.colorprint("results/sql.txt kaydedildi!")
